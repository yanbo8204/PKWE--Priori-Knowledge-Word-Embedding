...
class PrecompDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_split, vocab, embed_gen=False):
        self.vocab = vocab
        loc = data_path + '/'
        ...
        if data_split == 'dev':
         ...
         if data_split == 'train' and embed_gen:   
            self.length = len(self.captions)
            m = PCA(n_components=300)
            self.images = m.fit_transform(self.images)
    ...
    def __getitem__(self, index):
        ...
        caption.append(vocab('<end>'))
        target = torch.Tensor(caption)
        return image, target, index, img_id, tokens
...
def collate_fn(data):
    ...
    images, captions, ids, img_ids, tokens = zip(*data)
    ...
    return images, targets, lengths, ids, tokens
...
def get_precomp_loader(data_path, data_split, vocab, opt, batch_size=100,
    ...
    """Returns torch.utils.data.DataLoader for custom coco dataset."""
    dset = PrecompDataset(data_path, data_split, vocab, embed_gen=opt.embed_generate)
